...
define("text", ["module"], function(module) {
    ...
    }), define("text!../html/mcqtest.html", [], function() {
        ...
        var head = document.getElementsByTagName("head")[0],
            ...
            useImportLoad = !1,
            useOnload = !0;
        engine[1] || engine[7] ? useImportLoad = parseInt(engine[1]) < 6 || parseInt(engine[7]) <= 9 : engine[2] || engine[8] || "WebkitAppearance" in document.documentElement.style ? useOnload = !1 : engine[4] && (useImportLoad = parseInt(engine[4]) < 18);
        var cssAPI = {};
    ...
    }), define("css!../css/mcqtest", [], function() {}), define("mcqtest", ["text!../html/mcqtest.html", "css!../css/mcqtest.css"], function(mcqTemplateRef) {
        mcqtest = function() {
            "use strict";
            ...
            function init(elRoot, params, adaptor, htmlLayout, jsonContentObj, callback) {
                ...
                if (void 0 === jsonContent.content && (isContentValid = !1), !isContentValid) return void(callback && callback());
                ...
                processedHTML = processedHTML.replace(/&lt;/g, "<"), processedHTML = processedHTML.replace(/&gt;/g, ">"), $(elRoot).html(processedHTML), $(__constants.DOM_SEL_ACTIVITY_BODY).attr(__constants.ADAPTOR_INSTANCE_IDENTIFIER, adaptor.getId()), $("input[id^=option]").change(__handleRadioButtonClick), callback && callback()
            }
            ...
            function handleSubmit(event) {
                __saveResults(!0), activityAdaptor.showAnswers && __markAnswers(), $("input[id^=option]").attr("disabled", !0)
            }

            function showGrades(savedAnswer, reviewAttempt) {
                updateLastSavedResults(savedAnswer), __markAnswers(), $("input[id^=option]").attr("disabled", !0)
            }
            ...
            function __saveResults(bSubmit) {
                ...
                bSubmit === !0 ? activityAdaptor.submitResults(answerJSON, activityBodyObjectRef, function(data, status) {
                    status === __constants.STATUS_NOERROR ? (__state.activitySubmitted = !0, activityAdaptor.closeActivity(), __state.currentTries = 0) : __state.currentTries < __config.MAX_RETRIES && (__state.currentTries++, __saveResults(bSubmit))
                }) : activityAdaptor.savePartialResults(answerJSON, activityBodyObjectRef, function(data, status) {
                    status === __constants.STATUS_NOERROR && (__state.activityPariallySubmitted = !0)
                })
            }
            ...
            function __handleRadioButtonClick(event) {
                var currentTarget = event.currentTarget,
                    quesIndex = 0;
                $("label.radio").parent().removeClass("highlight"), $(currentTarget).parent().parent("li").addClass("highlight");
                var newAnswer = currentTarget.value.replace(/^\s+|\s+$/g, "");
                __content.userAnswersJSON[quesIndex] = newAnswer.replace(/^\s+|\s+$/g, ""), __state.radioButtonClicked = !0;
                __content.questionsJSON[0].split("^^")[2].trim()
            }

            function __markAnswers() {
                for (var radioNo = "", i = 0; i < __content.optionsJSON.length; i++) radioNo = "" + i, __markRadio(radioNo, __content.answersJSON[0], __content.optionsJSON[i])
            }

            function __markRadio(optionNo, correctAnswer, userAnswer) {
                userAnswer.trim() === correctAnswer.trim() ? ($("#answer" + optionNo).removeClass("wrong"), $("#answer" + optionNo).addClass("correct"), $("#answer" + optionNo).parent().addClass("state-success")) : ($("#answer" + optionNo).removeClass("correct"), $("#answer" + optionNo).addClass("wrong"), $("#answer" + optionNo).parent().addClass("state-error")), $("#answer" + optionNo).removeClass("invisible")
            }
            ...
            function __getAnswersJSON(skipQuestion) {
                var score = 0,
                    ...
                    results = {},
                    end_current_test = !1,
                    resultArray = new Array(1),
                    questionData = __content.questionsJSON[0].split("^^"),
                    interactionId = questionData[2].trim();
                return __constants.END_TEST && (end_current_test = !0), skipQuestion ? answer = "Not Answered" : (answer = __content.userAnswersJSON[0], __content.answersJSON[0] === __content.userAnswersJSON[0] && score++), results = {
            ...
            function __parseAndUpdateJSONContent(jsonContent, params) {
                jsonContent.content.displaySubmit = activityAdaptor.displaySubmit, __content.activityType = params.engineType;
                var tagName = jsonContent.content.instructions[0].tag;
                return __content.directionsJSON = jsonContent.content.instructions[0][tagName], jsonContent.content.directions = __content.directionsJSON, __parseAndUpdateQuestionSetTypeJSON(jsonContent), jsonContent
            }

            function __parseAndUpdateQuestionSetTypeJSON(jsonContent) {
                var interactionId = "",
                    interactionTag = "",
                    interactionReferenceString = "http://www.comprodls.com/m1.0/interaction/mcqsc",
                    parsedQuestionArray = $.parseHTML(jsonContent.content.canvas.data.questiondata[0].text);
                $.each(parsedQuestionArray, function(i, el) {
                    this.href === interactionReferenceString && (interactionId = this.childNodes[0].nodeValue.trim(), interactionTag = this.outerHTML, interactionTag = interactionTag.replace(/"/g, "'"))
                ...
                for (var questionText = "1.  " + jsonContent.content.canvas.data.questiondata[0].text, correctAnswerNumber = jsonContent.responses[interactionId].correct, interactionType = jsonContent.content.interactions[interactionId].type, optionCount = jsonContent.content.interactions[interactionId][interactionType].length, i = 0; i < optionCount; i++) {
                    var optionObject = jsonContent.content.interactions[interactionId][interactionType][i],
                        option = optionObject[Object.keys(optionObject)].replace(/^\s+|\s+$/g, "");
                    __content.optionsJSON.push(__getHTMLEscapeValue(option)), optionObject[Object.keys(optionObject)] = option, jsonContent.content.interactions[interactionId][interactionType][i] = optionObject, Object.keys(optionObject) == correctAnswerNumber && (__content.answersJSON[0] = optionObject[Object.keys(optionObject)])
                }
            ...
            function __getHTMLEscapeValue(content) {
                var tempDiv = $("<div></div>");
                return $(tempDiv).html(content), $("body").append(tempDiv), content = $(tempDiv).html(), $(tempDiv).remove(), content
            }
